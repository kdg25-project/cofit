...
export default function FriendListItem({ id, name, message, time, unread, onAvatarClick, isInParty = false }: FriendListItemProps) {
	...
	return (
		...
		>
			...
			<div 
				...
				className="relative flex-shrink-0 w-12 h-12 rounded-full bg-text flex items-center justify-center hover:ring-2 hover:ring-secondary transition-all"
			>
		...
		<div className="flex-1 min-w-0">
			<div className="flex items-center justify-between mb-1">
				<h4 className="text-base font-medium text-text">{name}</h4>
		...
		{/* 未読バッジ（通常時は右端に表示） */}
		{!isInParty && unread > 0 && (
			<div className="flex-shrink-0 w-6 h-6 rounded-full bg-notification flex items-center justify-center">
				<span className="text-text2 text-xs font-bold">{unread}</span>
			</div>
		)}
	</div>
